        ...
        <div class="flex justify-between gap-x-2">
          <div class="w-1/2 border-r pr-10">
            ...
            <div class="flex items-center justify-between gap-x-8 w-full my-3">
              <label for="NXB" class="block text-lg font-medium text-nowrap"
                >Nhà Xuất Bản</label
              >
              <select
                id="NXb"
              ...
              >
                ...
                <option
                  v-for="item in dataNXB?.data"
                  :key="item.maNXB"
                  :value="item.maNXB"
                  class="text-lg mx-2 my-3"
                >
                  {{ item.tenNXB }}
                </option>
              </select>
            ...
            <div class="my-5">
              ...
              >
                <div
                  v-if="data?.data.sach.photo != '' || photoURL"
                ...
                >
                  <img
                    ...
                    :src="
                      photoURL
                        ? photoURL
                        : stable.UPLOAD_FOLDER_BASE_URL + data?.data.sach.photo
                    "
                    alt=""
                  />
                </div>
                ...
                <span v-if="!data?.data.sach.photo && !photoURL">Upload file</span>
          ...
          <div class="w-1/2">
            ...
            <div class="mt-2">
              <label for="" class="block text-sm font-medium">Thể Loại:</label>
              <MultiSelect
                v-model="category"
                :options="cities"
                optionLabel="name"
...
export default {
  ...
  setup() {
    ...
    const { data } = useQuery(["sachEdit", maSach], () =>
      getDetailSach(maSach)
    );
    const { data: dataNXB } = useQuery(["getAllNXB"], () => getAllNXB(maSach));
    onMounted(async () => {
      const reponse = await axios.get(`/api/sach/${maSach}`);
      dataDefault.value = reponse.data.sach;
      category.value = dataDefault.value.category.map((item) => ({
        name: item.title,
        code: item._id,
      }));
    });
    ...
    watch(() => {
      ...
      maNXB.value = dataDefault.value.maNXB;
    });
    ...
    const handlerSubmitSach = async () => {
      ...
      try {
        ...
        await mutate({ maSach: maSach, formData: formData });
      } catch (error) {
